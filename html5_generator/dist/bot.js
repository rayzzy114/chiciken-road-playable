...
import { DEFAULT_CURRENCY, DEFAULT_STARTING_BALANCE, createInitialSession, getDiscount, calcPrice, buildOrderSummary, buildProfileMessage, parsePayCallback, getLibraryPath, } from "./bot_helpers.js";
...
async function orderWizard(conversation, ctx) {
    ...
    await ctx.reply("🔗 <b>Отправьте CTA-ссылку для редиректа</b>\nПример: <code>https://example.com</code>", { parse_mode: "HTML" });
    let ctaUrl = null;
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (ctaUrl)
            break;
        await ctx.reply("Некорректная ссылка. Отправьте валидный http/https URL, например https://example.com", { parse_mode: "HTML" });
    }
    if (!ctaUrl) {
...
export function registerHandlers(bot) {
    ...
    bot.callbackQuery(GAMES.PLINKO.ID, async (ctx) => {
        await ctx.answerCallbackQuery();
        await editOrReply(ctx, `<b>🎱 Classic Plinko</b>\n\nВ разработке! Скоро будет доступно. 🚧`, new InlineKeyboard().text("🔙 Назад", CATEGORIES.PLINKO));
    });
    // 4. Buy Check -> Wizard
    bot.callbackQuery(/^buy_check_/, async (ctx) => {
        ...
        if (!game)
            return editOrReply(ctx, "Некорректный выбор игры.", withBackToMenu);
        const pricing = await getEffectiveDiscountForGame(ctx.from.id, game.key);
    ...
    bot.callbackQuery(/^payment_cancel_/, async (ctx) => {
        ...
        await editOrReply(ctx, "Оплата отменена, заказ переведён в статус cancelled.", buildCancelledOrderKeyboard());
    });
    bot.callbackQuery(/^manual_pay_menu_/, async (ctx) => {
        ...
        await editOrReply(ctx, "Выберите тип прямой оплаты. После перевода отправьте TX hash или скриншот для ручной проверки.", new InlineKeyboard()
    ...
    bot.callbackQuery(/^manual_pay_(single|sub)_/, async (ctx) => {
        ...
        const message = `<b>Прямая оплата заказа ${orderId}</b>\n\n` +
            `<b>Сумма:</b> $${amount}\n` +
    ...
    bot.callbackQuery(/^manual_paid_(single|sub)_/, async (ctx) => {
        ...
        await editOrReply(ctx, "Отправьте TX hash текстом или скриншот фото/документом.\nЧтобы отменить, отправьте /cancel.", new InlineKeyboard().text("Главное меню", "main_menu"));
    });
    bot.on("message", async (ctx, next) => {
        ...
        const adminMessage = "<b>Получено подтверждение ручной оплаты</b>\n\n" +
    ...
    bot.callbackQuery(/^crypto_check_/, async (ctx) => {
        ...
        if (!orderId)
            return editOrReply(ctx, "Некорректная ссылка проверки оплаты.", withBackToMenu);
        const order = await DB.getOrder(orderId);
        if (!order)
            return editOrReply(ctx, "Заказ не найден.", withBackToMenu);
        ...
        if (!payment) {
            return editOrReply(ctx, "Инвойс не найден. Отмените оплату и создайте новый заказ.", buildCancelPaymentKeyboard(orderId));
        }
        if (order.status.startsWith("paid")) {
        ...
        try {
            ...
            if (!invoice) {
                return editOrReply(ctx, "Инвойс не найден в Crypto Pay. Отмените оплату и создайте новый заказ.", buildCancelPaymentKeyboard(orderId));
            }
    ...
    bot.callbackQuery(/^pay_/, async (ctx) => {
        ...
        if (isCryptoPayEnabled()) {
            ...
            try {
                ...
                await editOrReply(ctx, `Инвойс создан на $${amount}. Оплатите и нажмите «Проверить оплату».`, buildCryptoInvoiceKeyboard(parsed.orderId, invoice.payUrl));
            }
            catch (e) {
                console.error("Crypto invoice create error:", e);
                await editOrReply(ctx, "Не удалось создать инвойс Crypto Pay. Проверьте токен/настройки и попробуйте снова.", withBackToMenu);
            }
            return;
        }
        let alreadyPaid = false;
    ...
    bot.callbackQuery(/^admin_manual_(approve|reject)_/, async (ctx) => {
        ...
        try {
            await bot.api.sendMessage(Number(order.userId), `Оплата по заказу ${orderId} отклонена. Проверьте данные и отправьте новое подтверждение.`, { parse_mode: "HTML" });
        }
        catch (e) {
    ...
    bot.command("grantorder", async (ctx) => {
        ...
        if (!orderId)
            return ctx.reply("Использование: /grantorder <orderId>");
        const result = await approveManualOrder(orderId);
